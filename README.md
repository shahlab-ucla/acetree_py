...
- **Expression Comparison (Window → New Expression Comparison…)** - compare one exact canonical cell across any number of saved XML datasets. Assign replicates to conditions, globally show or hide partially transparent traces, and summarize each group with a mean plus SD/SEM/95% t-confidence band or a median plus IQR/scaled-MAD band. A dataset's first image recomputation makes one movie pass and caches every image channel plus raw/global/blot aggregates; changing the cell, channel, correction, or comparison window then reuses that shared session family without rereading the movie (`local`/`cross` use the documented global fallback). Tidy CSV preserves numeric and unavailable-status records; SVG saves the current figure when numeric data exist. **Save portable result…** writes a checksummed `.aceexpr` capture that can be reopened through **Window → Open Expression Result…**, **Open frozen result…**, or drag/drop, then retuned and exported without the source XMLs or images. Live source changes/reloads still fail closed and invalidate the session cache.
