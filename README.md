...
- **Expression Comparison (Window > New Expression Comparison...)** - compare one exact canonical cell across any number of saved XML datasets. Assign replicates to conditions, globally show or hide partially transparent traces, and summarize each group with a mean plus SD/SEM/95% t-confidence band or a median plus IQR/scaled-MAD band. Recomputed all-channel measurements are cached without mutating the datasets. Tidy CSV preserves numeric and unavailable-status records; SVG saves the currently rendered figure when numeric data exist. Multiple windows share the session cache, while unsaved main-viewer edits, changed sources, and an explicit reload block old exports until the affected row is saved/reloaded and prepared again.
